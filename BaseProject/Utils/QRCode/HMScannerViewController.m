...
- (void)viewWillAppear:(BOOL)animated {
    ...
    [self.navigationController.navigationBar setBarTintColor:RGBColor(51, 51, 51)];
...
- (void)prepareOtherControls {
    ...
    tipLabel.text = NSLocalizedString(@"WLQrcodeTips", nil);
...
//    self.navigationController.navigationBar.shadowImage = [[UIImage alloc] init];
    ...
    // 2> 标题
    self.title = NSLocalizedString(@"WLScanning", nil);
    
    // 3> 左右按钮
    self.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(@"WLClose", nil) style:UIBarButtonItemStylePlain target:self action:@selector(clickCloseButton)];
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(@"WLAlbum", nil) style:UIBarButtonItemStylePlain target:self action:@selector(clickAlbumButton)];

    [self.navigationItem.rightBarButtonItem setTitleTextAttributes:[NSDictionary dictionaryWithObjectsAndKeys:[UIFont boldSystemFontOfSize:16], UITextAttributeFont, [UIColor whiteColor], UITextAttributeTextColor, nil] forState:UIControlStateNormal];
    [self.navigationItem.leftBarButtonItem setTitleTextAttributes:[NSDictionary dictionaryWithObjectsAndKeys:[UIFont boldSystemFontOfSize:16], UITextAttributeFont, [UIColor whiteColor], UITextAttributeTextColor, nil] forState:UIControlStateNormal];
}
